...
sampler1=None 
with open('/albedo/work/projects/p_inverT/Ice_BT/output/sampler_output/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK.pkl', 'rb') as f:
    sampler1=dill.load(f)

sampler2=None
with open('/albedo/work/projects/p_inverT/Ice_BT/output/sampler_output/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_conti1.pkl', 'rb') as f2:
    sampler2=dill.load(f2)
    
sampler3=None
with open('/albedo/work/projects/p_inverT/Ice_BT/output/sampler_output/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_conti2.pkl', 'rb') as f3:
    sampler3=dill.load(f3)    
    
chains = []

for splr in [sampler1, sampler2,sampler3]:
    if splr is not None:
...
def select_ICs_p01mK(ics,T_val):
    st=np.round(T_val,4)
    first=float((str(st)[:7])) 
    second=float((str(st)[:7]))-float((str(0.001)[:7])) 
    if len(str(st)) > 7:
        needed=float((str(st)[7])) 
    else:
        return select_ICs(ics,st) 
    needed_ic=select_ICs(ics,first)-(select_ICs(ics,first)-select_ICs(ics,second))*needed*0.1
    return needed_ic
...
burnin=25000
total_plus_burn=150000


splrs = [sampler1, sampler2, sampler3]
...
chain_burned = full_chain[burnin:total_plus_burn, :, :]
log_prob_burned = full_log_prob[burnin:total_plus_burn, :]
...
#----------------------------------------------------------------------------saving_results------------------------------------------------------------------------------
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_POM_mean',POM_mean)
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_POM_map',POM_map)
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_ic',select_ICs_p01mK(ics,qmean[0]))
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_qmean',qmean)
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_qmap',qmap)
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_lower_bound',lower_bound)
np.save('/albedo/work/projects/p_inverT/Ice_BT/output/output_analysed/KS_final_runs/Realistic_org_rsignal_1_aprx_0mK_upper_bound',upper_bound)
